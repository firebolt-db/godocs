...
| `MAX_ERRORS_PER_FILE`    | Specify the maximum number of rejected rows per file. You can set this limit using either an integer (e.g., 0, 50, 100) or a percentage, but only 0% and 100% are valid percentage options. By default, the value is set to 0, meaning no errors are allowed. Conversely, setting it to 100% allows all errors. If the number of rejected rows surpasses the threshold, an error will be shown.

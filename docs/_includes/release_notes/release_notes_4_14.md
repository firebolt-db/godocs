...
**Added `scanned_storage_bytes` and `scanned_cache_bytes` to information schema views**

Added `scanned_storage_bytes` and `scanned_cache_bytes` columns to `information_schema.engine_query_history` and `information_schema.engine_user_query_history` views. These columns were previously accessible when explicitly used in a `SELECT` clause, but will now appear by default when you use `SELECT *`.

...
|--------------------------|---------------------------|
| US East (N. Virginia)    | $27.07                    |
| US West (Oregon)         | $27.07                    |
| Europe (Frankfurt)       | $28.84                    |
| Europe (Ireland)         | $27.07                    |
| Asia Pacific (Singapore) | $29.43                    |
| Asia Pacific (Mumbai)    | $29.43                    |
...
2. In the Firebolt Workspace, select the Configure(<img src="../../assets/images/configure-icon.png" width="20" alt="The Firebolt Configure Space icon" style="display: inline; margin-bottom: 0; margin-top: 0" />) icon from the left navigation pane.
...
storage_costs AS (
  SELECT
    ...
    SUM(consumed_gib_per_month) AS total_storage_gib
...
SELECT
  ...
  (total_storage_gib / EXTRACT(DAY FROM DATE_ADD('day', -1, CURRENT_DATE))) AS avg_daily_storage_gib
...
SELECT 
  ...
  SUM(consumed_gib_per_month) AS consumed_gb
FROM information_schema.storage_billing
WHERE DATE_TRUNC('month', usage_date) = DATE_TRUNC('month', CURRENT_DATE)
GROUP BY usage_date
ORDER BY usage_date DESC;
...
WITH
  ...
  storage_base AS (
    SELECT
      ...
      CAST(NULLIF(SUM(consumed_gib_per_month), 0) AS DOUBLE) AS consumed_gb
    FROM
...
WITH
  ...
  storage_base AS (
    SELECT
      ...
      SUM(IF(is_credit = FALSE, billed_cost, 0)) AS storage_cost,
      SUM(IF(is_credit = TRUE, billed_cost, 0))  AS storage_cost_credit,
      NULLIF(SUM(consumed_gib_per_month), 0)     AS consumed_gb
    FROM information_schema.storage_billing
    WHERE usage_date BETWEEN '2025-01-01' AND '2025-05-31'
    GROUP BY usage_date
  ),
  ...
  monthly_agg AS (
    SELECT
      ...
      SUM(fbu)                                          AS fbu,
      AVG(consumed_gb)                                  AS consumed_gb
    FROM daily_combined

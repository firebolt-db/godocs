...
| `<offset>`  | The number of rows backward from the current row from which to obtain a value. A negative number will act as [LEAD()](./lead.md). The offset must be a literal `INTEGER`.       | 	`INTEGER` |

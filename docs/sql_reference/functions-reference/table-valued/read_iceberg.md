...
A table-valued function (TVF) that reads data from [Apache Iceberg](https://iceberg.apache.org/) tables. The function can use either a [location object]({% link sql_reference/commands/data-definition/create-location.md %}) or individual TVF parameters to access the data, and can read from file-based catalogs as well as REST catalogs. `READ_ICEBERG` returns a table with data from the specified Iceberg table.

...
Returns the number of elements in the `<array>` array that match a specified function `<function>`. If you want only a count of the elements in an array without any conditions, we recommend using the [LENGTH](./length.md) function instead.
...
In this example below, there are no `<function>` criteria provided in the `ARRAY_COUNT` function. This means the function will count all of the non-false elements in the array. `0` is not counted because it evaluates to `FALSE`

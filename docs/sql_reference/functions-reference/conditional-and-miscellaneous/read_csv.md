...
| `<null_string>`               | Specify the string used to represent `NULL` values. The default is an empty string, which means that no specific null string is defined.                                                             | `TEXT`                |
...
| `<empty_field_as_null>`       | Specify whether empty fields should be interpreted as `NULL` values.                                                 | `BOOL`                |
...
**Query:**
In the following example, the `url` is set as the first positional parameter and reads a CSV file. However, `url` can also be passed as a named parameter using the syntax: `url => 's3://bucket/data.csv'`.
...
**Query:**
The following example reads a CSV with headers and will generate an error if any row contains a `NULL` value:

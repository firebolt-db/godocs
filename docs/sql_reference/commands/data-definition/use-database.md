...
## Using `USE DATABASE` with an engine's URL

When issuing a successful `USE DATABASE` statement using an engine's URL (see [information_schema.engines](../../../sql_reference/information-schema/engines.md)), Firebolt returns the `Firebolt-Update-Parameters` response header containing the new parameters that should be appended to the HTTP query string for subsequent statements to use the newly selected database.

```bash
curl --verbose "<engine_url>" --data-binary "USE DATABASE example"
...
# ...
curl --silent "<engine_url>" --data-binary "SELECT current_database()"
# Statement still returns previously selected database, there is no server-side state and we have not yet applied the HTTP query parameter returned from `USE DATABASE`.
curl --silent "<engine_url>/?database=example" --data-binary "SELECT current_database()"
# Statement returns "example".
```

## FAQs

1.	What is the purpose of the `USE DATABASE` command?<br>
...
2.	How does the `USE DATABASE` command affect the resolution of unqualified object names in queries?<br>
...
3.	Can I access objects in different databases without changing the session’s database context after using `USE DATABASE`?<br>
...
4.	What happens if I use another `USE DATABASE` command within the same session?<br>
...
5.	How do I ensure my query targets the correct database and object if I have used the `USE DATABASE` command?<br>
...
6.	Can I use `USE DATABASE` within a transaction?<br>

...
This guide explores how to import data from an AWS S3 bucket into Firebolt using `COPY FROM`. `COPY FROM` can accommodate different data loading workflows. For example, you can do the following:
* Load different tables simultaneously and in parallel. 
* Populate a single table from multiple sources and by multiple clients at once.
* Load partitioned data.
* Enable automatic schema detection when manual schema definition can be cumbersome and error-prone.
...
| `MAX_ERRORS_PER_FILE`    | Specify the maximum number of rows that can be rejected per file. `MAX_ERRORS_PER_FILE` can be integer or percentage in the format 'integer%', such as '100%'. By default, no errors are allowed. If `MAX_ERRORS_PER_FILE` is set to '100%', then all errors are allowed. If the threshold is exceeded, an error will occur.                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `QUOTE`                  | Specify the character used for quoting fields. The default is double quote (`"`). If a single quote is specified, the quote character will be `'`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `ESCAPE`                 | Specify the character used to escape special characters. The default character is quote (`) character.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |

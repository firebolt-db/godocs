...
When loading data into tables, you can filter data using the following options:
  ...
  2. `OFFSET`: Skips a specified number of rows in the final result set before ingestion. Behaves in the same way as the `OFFSET` clause in `SELECT` queries. 

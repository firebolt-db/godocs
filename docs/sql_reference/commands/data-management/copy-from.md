...
This guide explores how to import data into Firebolt using the COPY FROM command. You can use this command to efficiently ingest data from AWS S3 into Firebolt. The COPY FROM command offers straightforward syntax and accommodates various data-matching scenarios, providing flexibility in how you load your data. This feature is particularly useful in handling different situations that might arise during data import.
...
| `<default_value>`        | A replacement value for any NULL value generated by the source. This data type of `default_value` must be compatible with the data type of the target column.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `<source_column_index>`  | The index position of the column in the source data to be mapped. If you are specifying multiple source files, `source_column_index` specifies the index for all of those files. The index starts at 1, and can be as large as 2^64-1, to allow for a very large number of columns. If you prefix the index inside a command or query, preceed the column index with a dollar sign (`$`)character.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |

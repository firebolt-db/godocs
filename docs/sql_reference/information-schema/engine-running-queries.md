...
| e2e_duration_us | BIGINT      | The time elapsed in microseconds from when the query is submitted to the gateway to when the query on `information_schema.engine_running_queries` returns results. |

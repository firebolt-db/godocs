...
Creating struct literals is not yet supported. For information about how to ingest data of struct types, see [Ingesting struct values](#ingesting-struct-values).
...
Struct values can be ingested into a table by using the [`COPY FROM`](../sql_reference/commands/data-management/copy-from.html) statement. For example:
```sql
...
Other utilities like [`READ_PARQUET`](../sql_reference/functions-reference/table-valued/read_parquet.html) and [external tables](../sql_reference/commands/data-definition/create-external-table.html) do not support the `STRUCT` data type at the moment. We will extend the coverage in the following releases.
...
Struct columns can be used in [`GROUP BY`]({% link sql_reference/commands/queries/select.md %}#group-by) and [`ORDER BY`]({% link sql_reference/commands/queries/select.md %}#order-by) clauses of a [`SELECT`]({% link sql_reference/commands/queries/select.md %}) statement:
```sql
...
Firebolt supports both [`ARRAY`](../sql_reference/data-types.html#array) and `STRUCT` composite types. They compose freely, so you can model data as arrays of structs, structs of arrays, or more complicated shapes.
...
The `STRUCT` type composes with the [`ARRAY`](../sql_reference/data-types.html#array) type as well. It is possible to define an array of struct:
```sql
CREATE TABLE t3 (
    id  BIGINT,
    kvs ARRAY(
...
At the moment, [array functions](../sql_reference/functions-reference/array/) and [lambda functions](../sql_reference/functions-reference/Lambda/) generally do not support array of struct. The support will be extended in the near future.
...
Unnesting multiple arrays of structs together is supported, too. For more details on the array unnesting syntax, see [`UNNEST`](../Guides/loading-data/working-with-semi-structured-data/working-with-arrays.html#unnest).
...
Using [`UNNEST`](../Guides/loading-data/working-with-semi-structured-data/working-with-arrays.html#unnest) and [`ARRAY_AGG`](../sql_reference/functions-reference/aggregation/array-agg.html) together provides a fundamental way of working with arrays of struct even if there is no array or lambda function available for the operation in need.

### Transforming arrays of structs

The example query above can be simplified by using the [`ARRAY_TRANSFORM`](../sql_reference/functions-reference/Lambda/transform.html) function.

```sql
SELECT ARRAY_FILTER(y -> y > 10, ARRAY_TRANSFORM(x -> x.s_key, kvs)) FROM t3;
```

[`ARRAY_TRANSFORM`](../sql_reference/functions-reference/Lambda/transform.html) can be used to extract an attribute from an array of structs while keeping the array structure. Once having an array of primitive types, all other array functions can be used to process the array further. In this example, [`ARRAY_FILTER`](../sql_reference/functions-reference/Lambda/filter.html) is used to select keys that are bigger than 10.

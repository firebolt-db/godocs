...
2. [Load data into Firebolt](#load-data-into-firebolt) &ndash; Load exported data into Firebolt using the [COPY FROM](/reference-sql/commands/data-management/copy-from) command, using parallel ingestion and indexing to optimize loading speed and query performance.
...
Firebolt loads data directly from AWS S3 using the [COPY FROM](/reference-sql/commands/data-management/copy-from) command, supporting parallel ingestion, automatic schema discovery, metadata handling, and error management. It works with both CSV and Parquet formats, with Parquet recommended for efficient compression. You can use either the **SQL Workspace** or the **Load data wizard** to load data. For more information, see [Load data](/guides/loading-data).
...
TYPE = PARQUET;
```
For more information, see [Load data](/guides/loading-data).
...
* [Query plan insights](#firebolts-query-plan-insights) &ndash; Gain insights into query plans and how Firebolt's [EXPLAIN (ANALYZE)](/reference-sql/commands/queries/explain#example-with-analyze) command helps optimize queries through runtime metrics for optimal performance.
...
The same query can be run in Firebolt with no changes, as Firebolt supports standard SQL aggregate functions. To optimize performance, create an aggregating index in Firebolt that precomputes the total playtime for each `gameid`, avoiding runtime aggregation.
...
* [JSON_EXTRACT](/reference-sql/functions-reference/json/json-extract) &ndash; Extracts a part of the JSON data as raw JSON.
* [JSON_POINTER_EXTRACT_VALUES](/reference-sql/functions-reference/json/json-pointer-extract-values) &ndash; Extracts values from a JSON document using a pointer to the desired location.
...
* [JSON_VALUE](/reference-sql/functions-reference/json/json-value) &ndash; Extracts a JSON value and converts it to a SQL-compatible format like TEXT.
* [JSON_VALUE_ARRAY](/reference-sql/functions-reference/json/json-value-array) &ndash; Converts a JSON array into an array of SQL-compatible values.
* [JSON_POINTER_EXTRACT_KEYS](/reference-sql/functions-reference/json/json-pointer-extract-keys) &ndash; Extracts keys from a JSON document.
...
Hybrid functions, like [JSON_POINTER_EXTRACT_TEXT](/reference-sql/functions-reference/json/json-pointer-extract-text), combine both extraction and conversion, allowing access to nested JSON values. Hybrid functions in Firebolt combine extraction and conversion in a single operation. 
...
In Firebolt, the [EXPLAIN](/reference-sql/commands/queries/explain) command provides two key plans:
...
When paired with the [ANALYZE](/reference-sql/commands/queries/explain#example-with-analyze) option, Firebolt’s `EXPLAIN` command adds runtime metrics, such as CPU time, thread time, rows processed, and runtime for each operation, allowing for detailed performance analysis.
...
-- Create a large engine. By default, large and extra-large engines are limited to one node.
...
```
Once the data is in Amazon S3, use another Airflow task to run Firebolt’s [COPY FROM](/reference-sql/commands/data-management/copy-from) command to load the data into your Firebolt tables as follows:

...
1. Create a `docker-compose.yml` file with the following contents:
    ...
    ```
2. Replace placeholders `/path/to/your/jdbc/directory/` with the directory where you have placed the JDBC driver and `firebolt-jdbc-<version>.jar` with the name of the driver in that directory.
...
### Use the Connect API to add source and sink connectors
...
To get more information if something went wrong fetch Kafka Connect logs via `docker logs kafka-connect-1`.

### Write data and see it propagated

If everything succeeded you're now ready to send CDC data through the Kafka pipeline.
...
Kafka Connect is a framework that helps integrate Kafka with systems that are external to Kafka. To use Kafka Connect with Firebolt, you need to modify the Kafka connect properties, install the Debezium and Firebolt JDBC drivers, and create a sink connector for a Debezium sink connector.    
   ...
   3. **Configure a sink connector**    
    Use the sink-connector-config.json configuration file for the sink connector [created earlier](#firebolt-connector-configuration). Copy it into `config` directory of your Kafka installation or note its path for the next step.

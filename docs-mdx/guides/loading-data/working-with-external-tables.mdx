...
Firebolt supports loading data using *external tables*, which are different from [fact and dimension tables](/overview/indexes#firebolt-managed-tables). External tables store metadata objects that reference files stored in an Amazon S3 bucket, rather than actual data.

To create an external table, run the [CREATE EXTERNAL TABLE](/reference-sql/commands/data-definition/create-external-table) command. Firebolt recommends using [LOCATION objects](/guides/security/location) to securely manage credentials and connection details for external data sources. After you create an external table, use the [INSERT](/reference-sql/commands/data-management/insert) command to load the data from the external table into a fact or dimension table. Data that you ingest must be in the same AWS Region as the target Firebolt database.
...
The example below creates an external table that references an AWS S3 bucket that contains Parquet files from which Firebolt will ingest values for `c_id` and `c_name`. First, create a LOCATION object to securely store credentials:
...
Then create the external table using the LOCATION object:

...
Firebolt doesn't support an explicit `COLLATE` clause yet.
However, it's still possible to work with collations using the [`ICU_NORMALIZE`](/reference-sql/functions-reference/string/icu_normalize) scalar function.
This guide teaches you how to use this function to perform collation aware aggregations and joins.
...
You can see that you get one result row for every equlivalence class.
...
-- Aggregatin that uses the pre-normalized column
-- Timing: 0.09 seconds

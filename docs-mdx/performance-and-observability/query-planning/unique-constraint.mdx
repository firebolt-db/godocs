...
If the data in `c_custkey` is actually unique, the safe execution stategy above can lead to significant overhead. The first aggregation on `c_custkey` and the final restoring join are both a waste of time.

...
[The `EXLAIN` command](/reference-sql/commands/queries/explain) allows you to inspect the query plan at various stages in the query lifecycle.

1. The `EXPLAIN` output after the logical optimization phase: `EXPLAIN (logical) <query>`.
1. The `EXPLAIN` output after the physical optimization phase: `EXPLAIN (physical) <query>`.
1. The `EXPLAIN` output annotated with execution statistics: `EXPLAIN (analyze) <query>`.

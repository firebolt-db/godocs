...
* In the analyze output, we can see that the actual output cardinality of the filtered `store_sales` tablets is 4, which is the same as all tablets.
  ...
  The reason here is that the TPC-H data generator assigns `o_orderdate` values uniformly distributed in the `[1992-01-01, 1998-08-02]` range, so all of the 4 tablets contain relevant data.

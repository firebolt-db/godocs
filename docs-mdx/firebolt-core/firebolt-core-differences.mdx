...
* **At most one write transaction** can be active at a time in Firebolt Core (see also [Connect](/firebolt-core/firebolt-core-connect)). Attempting to start concurrent write transactions will result in an error.

...
You can run `helm update` to apply changes to your cluster after changing resources like memory or CPU allocation; you will need to manually delete all pods in such case:

```bash
kubectl delete pods -l app.kubernetes.io/name=firebolt-core
```
...
Followed by the deletion of all pods:
```bash
kubectl delete pods -l app.kubernetes.io/name=firebolt-core
```

This will trigger a cluster-wide update.

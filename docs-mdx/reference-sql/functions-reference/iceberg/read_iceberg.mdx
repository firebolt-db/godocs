...
- By default, cross-region reads for S3 are disabled for `read_iceberg`, as they may incur additional costs. They may be enabled on a per-query basis using the [cross_region_request_mode](/reference-sql/system-settings#access-cross-region-data) setting.

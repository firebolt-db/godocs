...
In the example below, the `TRANSFORM` function is used to [CAST](/reference-sql/functions-reference/conditional-and-miscellaneous/cast) each element from a string to a date type. With each element now as a date type, the [INTERVAL](/reference-sql/operators#arithmetic-with-date%2Ftime-and-intervals) function is then used to add 5 years to each.

...
If the pattern does not match, returns an empty array. If you want return the first match of `<pattern>` within the `<expression>`, use [REGEXP_EXTRACT](/reference-sql/functions-reference/string/regexp-extract).

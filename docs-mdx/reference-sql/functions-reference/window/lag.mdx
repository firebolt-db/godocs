...
| `<offset>`  | The number of rows backward from the current row from which to obtain a value. A negative number will act as [LEAD()](/reference-sql/functions-reference/window/lead). The offset must be a literal `INTEGER`.       | 	`INTEGER` |

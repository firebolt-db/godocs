...
It's similar to [arithmetic with intervals](/reference-sql/operators#arithmetic-with-date%2Ftime-and-intervals), but it also allows using a column reference for the `<quantity>`.

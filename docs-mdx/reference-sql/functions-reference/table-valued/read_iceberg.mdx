...
|:----------|:------------|:---------------------|
| `LOCATION` | The name of a location object that contains the Iceberg parameters and credentials. Firebolt recommends using `LOCATION` to store credentials for authentication. For a comprehensive guide, see [LOCATION objects](/guides/security/location). **Note:** `LOCATION` must be specified as a named parameter (e.g., `LOCATION => 'my_location'`). Unlike `URL`, it cannot be used as a positional parameter. | `TEXT` |
...
For a comprehensive guide to LOCATION objects, see [LOCATION objects](/guides/security/location). For syntax details, see [CREATE LOCATION](/reference-sql/commands/data-definition/create-location) and [CREATE LOCATION (Iceberg)](/reference-sql/commands/data-definition/create-location-iceberg).

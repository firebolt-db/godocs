...
|:----------|:------------|:---------------------|
| `LOCATION` | The name of a location object that contains Iceberg parameters and credentials. When specified, individual credential parameters are not required. For a comprehensive guide, see [LOCATION objects](/guides/security/location). | `IDENTIFIER` |
...
- Use a `LOCATION` object to store credentials for authentication. This approach centralizes credential management and eliminates the need to specify individual credential parameters in each query. See [CREATE LOCATION](/reference-sql/commands/data-definition/create-location) and [CREATE LOCATION (Iceberg)](/reference-sql/commands/data-definition/create-location-iceberg).

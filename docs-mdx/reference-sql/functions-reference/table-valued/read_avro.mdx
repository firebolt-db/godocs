...
|:----------|:--------------|:------|
| `null` | `TEXT` (nullable) | Standalone null columns are supported and handled as nullable string columns that are always null. Within unions, null specifies nullability for the resulting type |
| `boolean` | `BOOLEAN` | |
| `int` | `INT` | |
| `long` | `BIGINT` | |
| `float` | `REAL` | |
...
**Null types**: The `null` type is supported both as a standalone column and within unions. Standalone `null` columns are handled as nullable string columns that are always null. Within unions, `null` specifies the nullability of the resulting type itself (e.g., `["null", "string"]` creates a nullable string field, and `["null", "string", "int"]` creates a nullable struct where the entire struct can be null).

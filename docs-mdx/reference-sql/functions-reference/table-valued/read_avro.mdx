...
The result is a table with data from the Avro files. Columns are read and parsed using their inferred data types based on the Avro schema.
...
| `union` | `STRUCT` | Each union type becomes a nullable field in the struct |
...
**Unions**: Avro unions are converted to structs with nullable fields for each possible type. Field naming follows this convention:
...
SELECT * 
FROM READ_AVRO(
    URL => 's3://firebolt-publishing-public/help_center_assets/firebolt_sample_dataset/avro/simple.avro'
) 
LIMIT 3;
```
...
FROM READ_AVRO(
    URL => 's3://firebolt-publishing-public/help_center_assets/firebolt_sample_dataset/avro/map_as_record.avro'
) 
LIMIT 2;
```
...
FROM READ_AVRO(
    URL => 's3://firebolt-publishing-public/help_center_assets/firebolt_sample_dataset/avro/union_array_struct.avro'
) 
LIMIT 2;
```
...
```sql
SELECT * FROM READ_AVRO('s3://firebolt-publishing-public/help_center_assets/firebolt_sample_dataset/avro/simple.avro');
SELECT * FROM READ_AVRO(URL => 's3://firebolt-publishing-public/help_center_assets/firebolt_sample_dataset/avro/simple.avro');
```
...
```sql
SELECT *, $source_file_name FROM READ_AVRO('s3://firebolt-publishing-public/help_center_assets/firebolt_sample_dataset/avro/*.avro')
```
...
```sql
SELECT * FROM READ_AVRO('s3://firebolt-publishing-public/help_center_assets/*/avro/*.avro')
```
will read all Avro files in the bucket, including those in subdirectories like `firebolt_sample_dataset/avro/*.avro`. 
...
SELECT $source_file_name, "Name", "LevelType" FROM READ_CSV(
    LOCATION =>  'firebolt_sample_dataset', PATTERN => 'l*vels.csv',
    HEADER => true
) limit 5;
```
...
This reads only the CSV files matching the pattern `l*vels.csv` within the location's base path, showing specific columns and the source file for verification.

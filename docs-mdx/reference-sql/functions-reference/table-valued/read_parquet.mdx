...
READ_PARQUET (
  ...
  [, SUPPORT_STRUCTS => <support_structs>]
)
```
...
| `SUPPORT_STRUCTS` | A boolean which specifies whether structs in the source data are to be inferred as STRUCT data types. The default value is `FALSE`. For more information, see [Loading struct data](/reference-sql/data-types/struct#loading-struct-data). | `BOOL` |

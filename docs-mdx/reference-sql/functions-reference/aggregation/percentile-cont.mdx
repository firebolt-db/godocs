...
```sql
PERCENTILE_CONT(<percentile>) [FILTER ([WHERE] <condition>)] WITHIN GROUP (ORDER BY <expression> [{ASC | DESC}])
```
...
import SqlExamplesPercentileCont from '/snippets/sql_examples/percentile_cont.mdx';

<SqlExamplesPercentileCont  />

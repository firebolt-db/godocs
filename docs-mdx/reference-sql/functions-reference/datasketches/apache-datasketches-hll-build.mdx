...
function [APACHE_DATASKETCHES_HLL_MERGE](/reference-sql/functions-reference/datasketches/apache-datasketches-hll-merge).
To estimate the final distinct count value from a sketch, the scalar
function [APACHE_DATASKETCHES_HLL_ESTIMATE](/reference-sql/functions-reference/datasketches/apache-datasketches-hll-estimate) can be
used.
...
In the Druid Database they apply base64 encoding to the sketch blob, in order to make this function sketch compatible with theirs, you can use the following queries:

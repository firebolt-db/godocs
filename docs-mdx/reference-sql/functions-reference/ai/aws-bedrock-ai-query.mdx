...
### Update LOCATION to use access key and secret, then transform results

```sql
ALTER LOCATION my_bedrock_location SET
    CREDENTIALS = (
...
### Use role ARN with external ID

```sql
ALTER LOCATION my_bedrock_location SET
    CREDENTIALS = (
      AWS_ROLE_ARN='<aws_role_external_id>',

...
- All shapes that overlap a Polygon will be removed. This means that Polygons may be merged if they overlap and LineStrings may be cut to only include the parts outside of any Polygons:
  1. Input with overlapping Polygons and a LineString. <br/><img src="/assets/images/geography/normalization_before.png" alt="Overlapping Polygons with holes and a LineString crossing through them." style={{"width": "400px"}}/>
  2. In the output, the Polygons have been merged and the LineString was cut at the Polygon boundaries. <br/><img src="/assets/images/geography/normalization_after.png" alt="A single Polygon that combines the two input Polygons and three LineStrings covering the parts of the input LineString not covered by Polygons." style={{"width": "400px"}}/>
...
* Degenerate parts of Polygons are removed, potentially splitting the Polygon. Parts of Polygons are considered degenerate if they collapse to a single line or point:
  1. Input Polygon with a degenerate part. <br/><img src="/assets/images/geography/degenerate_part_removal_before.png" alt="A Polygon with a part that has no area." style={{"width": "400px"}}/>
  2. In the output, the degenerate part has been removed, turning the Polygon into a MultiPolygon. <br/><img src="/assets/images/geography/degenerate_part_removal_after.png" alt="The same Polygon split into two with the degenerate part removed." style={{"width": "400px"}}/>
...
* A special case of removing these degeneracies occurs when Polygon holes share a boundary edge with the Polygon. In this case, the hole is removed from the Polygon and the outer boundary is reshaped accordingly, but only if the edge vertices of the hole are also vertices of the Polygon:
  1. Input Polygon with a hole that shares two edges with the outer shell, making these edges degenerate. <br/><img src="/assets/images/geography/outer_and_inner_ring_share_edge_before.png" alt="A Polygon with a hole that shares two edges with the outer shell of the Polygon." style={{"width": "400px"}}/>
  2. In the output, the degenerate part has been removed. <br/><img src="/assets/images/geography/outer_and_inner_ring_share_edge_after.png" alt="The same Polygon but with the degenerate parts removed." style={{"width": "400px"}}/>

...
Parquet files with `STRUCT` types can be handled using both [`COPY FROM`](/reference-sql/commands/data-management/copy-from) and [`READ_PARQUET`](/reference-sql/functions-reference/table-valued/read_parquet) statements. 

[`COPY FROM`](/reference-sql/commands/data-management/copy-from) ingests data into a table, while [`READ_PARQUET`](/reference-sql/functions-reference/table-valued/read_parquet) allows querying external data directly — useful for filtering, transformations, or joining before ingesting.
...
SELECT * FROM read_parquet('s3://path/to/file.parquet, support_structs => true);
```

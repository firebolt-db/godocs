...
Other utilities like [`READ_PARQUET`](/reference-sql/functions-reference/table-valued/read_parquet) and [external tables](/reference-sql/commands/data-definition/create-external-table) do not support the `STRUCT` data type at the moment. We will extend the coverage in the following releases.
...
Unnesting multiple arrays of structs together is supported, too. For more details on the array unnesting syntax, see [`UNNEST`](/guides/loading-data/working-with-semi-structured-data/working-with-arrays#unnest).

### Aggregating structs into an array of struct

Some aggregate functions support structs. In particular, [`ARRAY_AGG`](/reference-sql/functions-reference/aggregation/array-agg) can be useful when working on array of struct, together with [`UNNEST`](/guides/loading-data/working-with-semi-structured-data/working-with-arrays#unnest).
...
Using [`UNNEST`](/guides/loading-data/working-with-semi-structured-data/working-with-arrays#unnest) and [`ARRAY_AGG`](/reference-sql/functions-reference/aggregation/array-agg) together provides a fundamental way of working with arrays of struct even if there is no array or lambda function available for the operation in need.

...
Firebolt supports [`FROM` first queries](/reference-sql/commands/queries/select#from-first), which are especially useful with pipe syntax, which builds queries in a step-by-step flow, with each step transforming the result of the previous one. Starting with the `FROM` clause defines the initial data source, allowing you to clearly chain joins, filters, and transformations.
...
The `<select_list>` in the `SELECT` pipe operator follows the same syntax as the [`SELECT` statement](/reference-sql/commands/queries/select#select-1) in standard SQL. Specifically, it can contain components including column references, scalar expressions, `*` wildcards, and the `EXCLUDE` clause.
...
The `WHERE` pipe operator filters the input data based on a specified condition, similar to the [WHERE](/reference-sql/commands/queries/select#where) clause in standard SQL, and also replaces the `HAVING` clause in pipe syntax, allowing you to exclude rows that don't meet the criteria.
...
The `LIMIT` pipe operator restricts the number of rows in the result set, similar to the [LIMIT clause](/reference-sql/commands/queries/select#limit) in standard SQL, and can optionally use `OFFSET` to skip a specified number of rows, useful for limiting large result sets, pagination, or testing smaller data subsets.
...
You can use the `AGGREGATE` pipe operator to perform either **full table aggregation** or **aggregation across groups**, similar to using the [`GROUP BY`](/reference-sql/commands/queries/select#group-by) clause in standard SQL. Use this operator to apply aggregate functions like `SUM`, `AVG`, or `COUNT` to grouped data. 
...
Use the `ORDER BY` pipe operator to sort the input data based on one or more expressions, similar to the [`ORDER BY`](/reference-sql/commands/queries/select#order-by) clause in standard SQL to organize query results in a meaningful order. You can use `ORDER BY` to specify the sorting order with options for ascending (`ASC`) or descending (`DESC`) order, as well as the handling of `NULL` values with `NULLS FIRST` or `NULLS LAST`. 
...
### `JOIN` pipe operator
Use the `JOIN` pipe operator to combine rows from two or more tables based on a related column and merge datasets in a query  similar to the standard SQL [`JOIN`](/reference-sql/commands/queries/select#join) clause. You can perform different types of joins, such as `INNER`, `LEFT`, `RIGHT`, and `FULL`, to retrieve and merge data based on specific conditions.

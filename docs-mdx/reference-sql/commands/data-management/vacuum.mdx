...
`VACUUM` improves query efficiency by restructuring tablets for optimal performance. DML operations such as [DELETE](/reference-sql/commands/data-management/delete), [UPDATE](/reference-sql/commands/data-management/update), [INSERT](/reference-sql/commands/data-management/insert), and [COPY FROM](/reference-sql/commands/data-management/copy-from) might create tablets that are not optimally sized. Suboptimal tablets occur because DML efficiently utilizes resources in proportion to the cardinality of the data being inserted. In addition to standard SQL operations, tuples that are deleted by an update are not always physically removed from their table; they remain present until a `VACUUM` is finished operating. In other words, tablets are not necessarily optimal for running queries; therefore, it’s necessary to run `VACUUM` periodically, especially on frequently updated tables.
...
By default, auto-VACUUM tasks affect stopping behavior. Specifically, the [AUTO_STOP](/reference-sql/commands/engines/create-engine#create-an-engine-with-auto-stop) timer is reset after auto-VACUUM tasks complete. Similarly, [STOP ENGINE](/reference-sql/commands/engines/stop-engine) will block until auto-VACUUM tasks completes. You can disable waiting for auto-VACUUM using the `ALTER ENGINE` statement:
...
|`job_type`         |The type of vacuum job. One of `MERGE`, `COMPACTION` or `RECREATE` |

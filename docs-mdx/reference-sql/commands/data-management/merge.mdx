...
The clauses `INSERT *` and `UPDATE SET *` can be used to ingest or overwrite rows without any data transformation. Note that these replacements are allowed _only_ when the source and target tables have precisely matching column names.

...
`MERGE` allows users to perform multiple data modifications ([`INSERT`](https://docs.firebolt.io/reference-sql/commands/data-management/insert), [`UPDATE`](https://docs.firebolt.io/reference-sql/commands/data-management/update), [`DELETE`](https://docs.firebolt.io/reference-sql/commands/data-management/delete)) within a single transaction. `MERGE` is useful for common database tasks like removing duplicates, syncing data between tables (upsert), and cleaning out old records.
...
There can be multiple branches defined for rows within each category. However at most one branch's action will be executed per row, similar to a [`CASE`](https://docs.firebolt.io/reference-sql/functions-reference/conditional-and-miscellaneous/case) statement. `WHEN` clauses are evaluated in order within each category, and the chosen action for each row will come from whichever branch condition matches first (or from the first branch without any condition). If none of the conditions match, no action is performed for that row.
...
| `<public_settings>` | List of [query-specific public settings](/reference/system-settings#setting-via-with) overrides. |
...
```
```sql
-- [Public Preview] VALUES subquery is not yet available on multi node engines
...
## Limitations
- `WITH` clause is not supported, for specifying one or more subqueries ([CTE](https://docs.firebolt.io/reference-sql/commands/queries/select#materialized-common-table-expressions)) that can be referenced from inside the `MERGE` query.
- `VALUES` clauses or `DIMENSION` tables as `<data_source>` on multi node engines are not yet supported.


## Related Topics

* [INSERT Statement](https://docs.firebolt.io/reference-sql/commands/data-management/insert)
* [UPDATE Statement](https://docs.firebolt.io/reference-sql/commands/data-management/update)
* [DELETE Statement](https://docs.firebolt.io/reference-sql/commands/data-management/delete)

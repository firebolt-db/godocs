...
Change the owner of a location. The current owner of a location can be viewed in the [information_schema.locations](/reference-sql/information-schema/locations) view on `location_owner` column.
...
```sql
ALTER LOCATION [IF EXISTS] <location_name> OWNER TO <user>
```

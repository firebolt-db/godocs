...
| `<partition_expression>` | An expression using [Iceberg partition transform functions](/reference-sql/functions-reference/iceberg) to define how data is partitioned. |
...
The following example creates a partitioned Iceberg table using [partition transform functions](/reference-sql/functions-reference/iceberg):
...
You can also use multiple partition columns:

```sql
CREATE ICEBERG TABLE my_multi_partitioned_table
  PARTITION BY (iceberg_year(event_date), iceberg_bucket(user_id, 16))
  AS SELECT event_id, event_date, user_id, event_data FROM my_events_table
WITH LOCATION = my_location;
```

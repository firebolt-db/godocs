...
| `SOURCE` | The external data source type. Currently, `AMAZON_S3` (See [CREATE LOCATION (Amazon S3)](/reference-sql/commands/data-definition/create-location-s3)) and `ICEBERG` (See [CREATE LOCATION (Iceberg)](/reference-sql/commands/data-definition/create-location-iceberg)) are supported. |
...
`LOCATION` objects are managed using [RBAC](/overview/security/rbac) with the following permission levels:

...
| `<aggregations>`    | [Aggregate functions](/reference-sql/functions-reference/aggregation) including `SUM`, `COUNT`, `AVG`, applied to table column expressions.  |
...
SELECT
  ...
  COUNTING_HLL_COUNT_DISTINCT(brand)
FROM
  my_table
WHERE
...
GROUP BY
  product_name;
```
To optimize the previous query, create an aggregating index on `my_table` that precomputes `COUNTING_HLL_COUNT_DISTINCT(brand)`, grouped by `origin_country`, `amount`, and `product_name` as follows:
...
<Note>
For tables with frequent `DELETE` operations, consider using [`COUNTING_HLL_COUNT_DISTINCT`](/reference-sql/functions-reference/aggregation/counting-hll-count-distinct) instead of `APPROX_COUNT_DISTINCT` in aggregating indexes.
`COUNTING_HLL_COUNT_DISTINCT` efficiently handles deletions without requiring full table scans, maintaining query performance even with frequent data modifications.
</Note>
...
You can use [EXPLAIN](/reference-sql/commands/queries/explain) to check if Firebolt is applying an aggregating index as shown in the following code example:
...
* **Prioritize low-cardinality columns** &ndash; Generally, place columns with fewer unique values first to improve pruning efficiency, though high-cardinality columns can be beneficial when they have selective filters.

...
| `<column_name>` | Name of the column to add to the table. |
...
| Parameter          | Description                                  |
| :------------------ | :-------------------------------------------- |
| `<table>`       | Name of the table from which to drop the column. |
| `<column_name>` | Name of the column to remove from the table. |
| `IF EXISTS` | If specified, this clause prevents an error message that would occur if the column does not exist in the table. |
...
* The column must not be used by any aggregating index on the table (as a grouping key or within the index's aggregation expressions). Drop or alter those indexes first. See [DROP INDEX](/reference-sql/commands/data-definition/drop-index).
...
```sql
ALTER TABLE my_table DROP COLUMN IF EXISTS optional_column;
```
...
| Parameter | Description |
| :-------- | :---------- |
| `<table_name>` | Name of the table containing the column to rename. |
| `<old_column_name>` | Current name of the column to rename. |
| `<new_column_name>` | New name for the column. |
| `IF EXISTS` (table level) | Prevents error if table doesn't exist. |
| `IF EXISTS` (column level) | Prevents error if column doesn't exist. |
...
```sql
ALTER TABLE users RENAME COLUMN IF EXISTS middle_name TO mi;
```
...
```sql
ALTER TABLE [IF EXISTS] <table> OWNER TO <user>
```

...
| `RELEASE = (CHANNEL = <channel>)`    | Specifies the release channel to be used. Options are `DEFAULT` to use the default release process or `PREVIEW` to get upgrades earlier. |

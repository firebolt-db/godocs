...
| peak_memory_bytes          | BIGINT      | Peak memory usage by the query. For distributed queries represents the peak memory used by any node. |
...
| transaction_begin_lsn      | TEXT        | Identifier for the query's transaction beginning (Log Sequence Number). |
| transaction_commit_lsn     | TEXT        | Identifier for the query's transaction commit (Log Sequence Number). |

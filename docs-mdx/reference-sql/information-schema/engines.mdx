...
| auto_vacuum_wait_on_stop    | BOOLEAN     | Indicates whether [AUTO_VACUUM_WAIT_ON_STOP](/reference-sql/commands/data-management/vacuum) is enabled or disabled by the user. Could also be `NULL` to indicate that `AUTO_VACUUM_WAIT_ON_STOP` is automatically enabled by default. |

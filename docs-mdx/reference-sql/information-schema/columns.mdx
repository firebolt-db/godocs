...
| compression               | TEXT    | The compression codec(s) applied to the column. Shows the full compression chain if multiple codecs are used (e.g., `delta, zstd(3)`). |
...
### Example: Viewing compression settings

To view compression settings for columns in a specific table:
...
Example output:
```
column_name    | data_type         | compression        | is_nullable
---------------|-------------------|--------------------|-----------
id             | INTEGER           | lz4                | NO
sensor_id      | INTEGER           | delta, lz4         | NO  
temperature    | DOUBLE PRECISION  | gorilla, zstd(3)   | YES
timestamp      | TIMESTAMPTZ       | doubledelta, lz4   | NO
metadata       | TEXT              | zstd(15)           | YES
```

...
The `information_schema.transitive_applicable_roles` view lists the roles that are granted to the current user, as well as those the user either owns or has the [necessary privilege to manage](/overview/security/rbac/role-permissions#role-permissions).

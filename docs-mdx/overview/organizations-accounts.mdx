...
* **Account** - A group of resources which can include a database, an engine and several users with associated logins or service accounts and roles. See the following [Accounts](#accounts) section for high-level information about accounts and [Manage accounts](/guides/managing-your-organization/managing-accounts) for more detailed information. Under account are the following levels:
    ...
    * **Location** - A secure, reusable object that stores connection details and credentials for external data sources. Locations provide centralized credential management and strengthen security by eliminating the need to embed credentials in queries or table definitions. For more information about locations, see [LOCATION objects](/guides/security/location).
...
When you [register for the first time](/guides/managing-your-organization/creating-an-organization), Firebolt sets up an organization for you. During registration, you’ll set up your first account, with one user. The first user that is added is the account administrator, as shown in the following diagram:
 ...
 <img src="/assets/images/manage-organization-first-registration.png" alt="When you first register, Firebolt sets up an organization with one account and user that has account administrator privileges." style={{"display": "block", "margin": "0 auto", "width": "400px"}}/>
...
<img src="/assets/images/manage-organization-accounts-regions.png" alt="An account can span only one AWS region." style={{"width": "400px"}}/>
...
<img src="/assets/images/manage-organization-accounts-contents.png" alt="In Firebolt, an account can contain many users, an engine and database." style={{"width": "400px"}}/>
...
<img src="/assets/images/user_login_service-account.png" alt="A user must be associated with either a login or a service account." style={{"width": "400px"}}/>
...
<img src="/assets/images/manage-organization-marketing-accounts-example.png" alt="A user can have multiple roles in an account." style={{"width": "400px"}}/>
...
Firebolt assigns two built-in system databases: `account_db` and `org_db`. These databases are automatically created for every account and organization respectively and are designed to provide a clear, hierarchical model for accessing system-level metadata. Both are strictly read-only, containing only system views. Their primary purpose is to help maintain the system model by relating metadata to the correct organizational level (Account or Organization).

### Syntax example

To retrieve all roles defined at the account level:

```sql
SELECT * FROM account_db.information_schema.enabled_roles;
```

To see a list of all accounts within your organization:

```sql
SELECT * FROM org_db.information_schema.accounts;
```

### The account database (account_db)

The `account_db` is scoped specifically to the current account. It contains system metadata relevant only to the administrative and operational scope of that individual account.

<Note>
If the request does not specify a database and no default database is configured for the connecting engine, `account_db` will be used as the default fallback database.
</Note>

The views within `account_db.information_schema` expose details about resources and configurations that are managed at the account level, such as:

* **Catalogs** – All databases within the account
* **Users** – User accounts and permissions
* **Transactions** – Transaction history and details
* **Engine metrics views** – Engine performance data (for example, `engine_history`)
* **Storage metric views** – Storage usage and history (for example, `storage_history`)

### The organization database (org_db)

The `org_db` is scoped to the entire organization that the current account belongs to. It contains higher-level, cross-account metadata relevant to the overall corporate entity.

The views within `org_db.information_schema` expose details about entities that exist at the organization level, such as:

* **Accounts** – All accounts within the organization
* **Invoices** – Billing and invoice information
* **Logins** – Login activity across the organization
* **Billing** – Organization-wide billing details

## Information schema visibility hierarchy

It is important to note that the `information_schema` views follow a hierarchical visibility model.

The `information_schema` in a regular user database provides a unified view of system metadata, consolidating information from its own scope and the upper system levels:

* **Database level** – Metadata specific to the current database (for example, only its tables, functions)
* **Account level** – Metadata from the `account_db` (for example, all databases in the account)
* **Organization level** – Metadata from the `org_db` (for example, all accounts in the organization)

This hierarchy means that a user querying their database's `information_schema` will see an aggregate list of system objects relevant to all three levels, ensuring comprehensive system awareness without needing to switch contexts.

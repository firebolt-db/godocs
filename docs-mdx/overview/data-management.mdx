...
Serializable isolation means that the result of running concurrent statements is equivalent to as if they were run serially in some order.
...
Firebolt is optimized for high read transaction rates and a low write transaction rates - each write transaction can be huge though. Design processes to **commit transactions at most once per second** for optimal performance. Each DML and DDL statement runs in an isolated transaction by default. `BEGIN` and `COMMIT` explicitly define transaction boundaries to achieve large or complex write operations in a single atomic step.

## Creating tables
Let’s look at an example of how to [create a simple table](/reference-sql/commands/data-definition/create-fact-dimension-table) called rankings.
```sql
...
## Inserting data
To support loading data from a data lake (Amazon S3) into Firebolt tables, Firebolt provides utilities for loading data in bulk. Users can perform both initial and incremental data ingestion using either a [COPY](/reference-sql/commands/data-management/copy-from) statement or [INSERT INTO SELECT FROM \<external_table\>](/reference-sql/commands/data-management/insert) syntax. 
...
To achieve the best throughput, **insert data in large to medium sized transactions**. Firebolt supports long-running transactions, and can combine DDL and DML statements in a single transaction.
...
Apart from tablet fragmentation, a singleton insert statement by default commits a write transaction. Design insertion processes to **commit at most one transaction per second** to avoid throttling. It is possible to batch many singleton insert statements into one transaction using `BEGIN` and `COMMIT`, but this does not prevent the tablet fragmentation issue described above.

## Deleting data
Firebolt supports storing as much data as needed for as long as needed. However, there are situations where data does need to be deleted. Situations like data corrections that occur in the systems of records, or GDPR compliance where a single (or multiple) customer record(s) must be deleted to preserve privacy, have led to support for [DELETE](/reference-sql/commands/data-management/delete) statements in Firebolt.
...
By default, a `DELETE` statement runs isolated in its own transaction. Design processes to commit at most one transaction per second to avoid throttling. `BEGIN` and `COMMIT` can be used to batch multiple statements into one transaction.

Deletion may conflict with other operations, notably with `UPDATE`s and other `DELETE`s, if the same tablets are affected, even if working on distinct rows. Design processes to avoid high rates of conflicting writes for optimal performance.

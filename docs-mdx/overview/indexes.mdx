...
* **Column order** &ndash; The order of columns in a composite primary index is critical. Place the lowest cardinality columns first in a composite primary index. Firebolt will search based on the order of columns in the primary index. Placing low-cardinality columns first results in broader row ranges per indexed value, enabling more efficient pruning during query execution. Once relevant ranges are selected using the leading (low-cardinality) columns, higher-cardinality columns further narrow the scan.
...
If you understand your data and query patterns, you should select low-cardinality columns frequently used in `WHERE`, `JOIN`, or `GROUP BY` clauses to minimize the amount of data scanned and improve query efficiency.

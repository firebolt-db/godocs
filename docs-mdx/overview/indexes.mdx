...
* **Column order** &ndash; The order of columns in a composite primary index is critical. Place the least selective, or lowest cardinality columns first in a composite primary index. Firebolt will search based on the order of columns in the primary index. Low-cardinality columns, which have few distinct values, improve index efficiency by creating long ordered runs of data that reduce the number of tablet ranges that need to be scanned during queries.

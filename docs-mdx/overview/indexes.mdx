...
* **Column order** &ndash; The order of columns in a composite primary index is critical. Place the lowest cardinality columns first in a composite primary index. Firebolt will search based on the order of columns in the primary index. A Primary Index column can only be used for pruning if all values of previous primary Index columns within a granule are equal. Therefore, placing low-cardinality columns first enables more efficient pruning on following columns.

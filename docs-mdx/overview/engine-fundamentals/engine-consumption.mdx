...
<Note>
The maximum number of nodes per engine depends on the instance type, family, and region, and may also be constrained by availability. If you require engine sizes beyond these limits, please contact support@firebolt.io.
</Note>
...
- If you create an engine with the following configuration: TYPE = “M”, NODES = 3, CLUSTERS=2, it will have 96 FBUs available per hour (16 x 3 x 2). 
- If you scale down the engine to use a “S” type node, the available FBUs per hour will decrease to 48 FBUs (8 x 3 x 2).

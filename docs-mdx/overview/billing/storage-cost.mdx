...
You can use data from the [`information_schema.storage_billing`](/reference-sql/information-schema/storage-billing) and [`information_schema.storage_history`](/reference-sql/information-schema/storage-history) views to analyze and optimize storage costs across different regions, accounts, and data usage trends. The following queries will guide you in tracking storage expenses, understanding regional cost variations, and identifying storage inefficiencies.
...
Use [`information_schema.storage_billing`](/reference-sql/information-schema/storage-billing) inside a query to understand which regions incur the highest storage costs, helping you optimize resource allocation based on geographical pricing differences. 

...
* [Altering primary indexes](#altering-primary-indexes)
...
* **Tablet metadata compatibility**: If your table contains older tablets that don't have the primary index information in their metadata headers, you must run `VACUUM` on the table before altering the primary index.
...
* **Column validation**: All specified columns must exist in the table. You'll receive an error if any column doesn't exist.

* **Metadata-only operation**: The ALTER TABLE operation only updates the table's metadata definition. Existing data in tablets retains the original primary index structure until `VACUUM` is run.
...
After altering a table's primary index, existing tablets will continue to use their original primary index structure. To ensure all data uses the new primary index definition:

1. Run the ALTER TABLE statement to update the metadata
2. Execute `VACUUM <table_name>` to reorganize existing tablets with the new primary index structure

The `VACUUM` operation will reorganize and merge tablets to align with the new primary index definition, ensuring optimal query performance across all data.

### Useful Links

For additional information, see [ALTER TABLE SET PRIMARY INDEX](/reference-sql/commands/data-definition/alter-table#alter-table-set-primary-index) reference page.
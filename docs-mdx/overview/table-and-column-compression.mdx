...
Firebolt allows users to specify custom compression algorithms at both the table level and column level when creating tables. 
Compression helps you reduce storage costs and improve query performance by decreasing disk usage and I/O operations.
...
### Specialized codecs
These preprocessing codecs optimize specific data patterns:

- `delta` – Stores differences between consecutive values; excellent for sequential numeric data (requires NOT NULL)
...
-- Specialized codecs require chaining with general-purpose codecs
...
**Chaining rules:**
- You can chain up to three codecs maximum
- Only one general-purpose codec is allowed per chain
...
<Note>
Delta codec requires `NOT NULL` columns since null values break its compression algorithms.
</Note>

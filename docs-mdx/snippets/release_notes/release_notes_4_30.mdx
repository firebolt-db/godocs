...
You can enable this by using [`alter table ... add statistics`](/performance-and-observability/query-planning/automated-column-statistics/table-with-statistics)
or by enabling the planner to [read from user-created aggregating indexes](/performance-and-observability/query-planning/automated-column-statistics/user-created-indexes).
...
{/* Jira ticket: FIR-49857 */}
{/* Jira ticket: FIR-50921 */}
**Type-based NDV Heuristics**

The query optimizer now assumes the number of distinct values (NDV) for columns based on their data types.
Users can disable this feature by setting `enable_type_based_ndv_heuristic` to `false`.
...
This can potentially help the query optimizer make better decisions. 
Read more about [datetime function NDV heuristics](/reference/system-settings#enable-datetime-function-ndv-heuristic) and [type based NDV heuristic](/reference/system-settings#enable-type-based-ndv-heuristic)

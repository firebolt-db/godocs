...
For file-based Apache Iceberg catalogs without a `version-hint.text` file, you can now enable Firebolt to [attempt to guess the correct metadata file to read](/reference/system-settings#guessing-iceberg-metadata-version) by setting `iceberg_unsafe_version_guessing_mode` to `version_num`. This behavior is disabled by default.

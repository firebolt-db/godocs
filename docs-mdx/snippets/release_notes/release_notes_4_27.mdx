...
Filtered Parquet scans now utilize embedded column minimum and maximum metadata to remove row groups that do not meet filter conditions. This can significantly reduce the amount of data scanned and is available for all Parquet scans ([`READ_ICEBERG`](/reference-sql/functions-reference/table-valued/read_iceberg), [`READ_PARQUET`](/reference-sql/functions-reference/table-valued/read_parquet), and external tables). To see whether Parquet pruning is used, consult `EXPLAIN (PHYSICAL)` or `EXPLAIN (ANALYZE)` and look for a `pruning_predicate` in the `read_from_s3` TVF's arguments.

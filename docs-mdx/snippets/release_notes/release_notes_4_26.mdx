...
**Added support for `FILTER` clause in aggregation functions**

The `FILTER` clause enables conditional aggregation - only values for which the condition evaluate to `true` are considered by the aggregation function.
